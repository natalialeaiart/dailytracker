...
A small offline app for tracking what you actually do each day — sport, study, job search, hobbies, anything you decide matters. It ships empty: you create your own categories and metrics on the first day. It runs entirely in the browser: no accounts, no server, no analytics. Your data never leaves your phone.
...
- The PDF report is for reading, not for restoring.

To save the monthly report: open **Stats → Report**. On iPhone tap Share → Print, then pinch the preview to save as PDF. On Android tap Print → Save as PDF.

**Reset tracker** (Settings) deletes every metric and all history but keeps your PIN. Back up first — it cannot be undone.
...
1. Create a new repository and add all files from this folder: `index.html`, `manifest.json`, `sw.js`, the four `icon-*.png` files, `README.md`, `.github/workflows/deploy.yml`.
...
Metric types: `check` (yes/no), `minutes`, `minutes_note`, `number`, `number_note` (number plus a short text), `note`. Categories are stored in settings and can be renamed, reordered, and given an icon.
...
| 2 | Calendar, metric editor (rename, goal, category, reorder, archive), streaks, notes feed | done |
| 2b | Custom categories with icons, minutes + note type, day-score toggle for yes/no metrics, app icon and offline install files | done |
| 3 | Statistics: month heatmap, per-zone table with streaks and goal rates | done |
| 4 | Monthly PDF report via browser print | done |
| 4b | Two themes (dark by default, lavender option), progress ring, print fixes | done |
| 4c | Named Dayta, new icon set, wordmark and tagline | done |
| 5 | Onboarding with presets, home-screen prompt, backup reminder | planned |
| 6 | Share your metric setup via link | planned |

## Themes

Two themes ship with the app: **Dark** (default) and **Lavender**. Switch in Settings — the choice is stored per device. The monthly report always prints as dark text on white paper regardless of the theme.
